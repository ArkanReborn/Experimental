...
temp = int(
    input(
        "Enter current Farenheit temperature, must be value >= -58 and <= 50 degrees: "
    )
)
...
if (temp < -58 or temp > 50) and ws < 3:
    ...
    print(
        f"Based on the entered temperature {temp:.2f} and wind speed of {ws:.2f} the currently calculated wind-chill is calculated to be: {chill_form:.2f}"
    )
